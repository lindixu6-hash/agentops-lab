...
- [带 Attestation 的 Producer/Verifier 运行](https://github.com/lindixu6-hash/awesome-agentic-engineering/actions/runs/31981738763)
...
Producer Commit 2255540
  ...
  └─ 上传一个 Producer Artifact
                  │
                  ▼
Reusable Verifier 锁定到 Commit 8d4b435
  ├─ 检出锁定到 Commit 34b1235 的 Verifier 代码
  ├─ 只下载一个名称确定的 Bundle
  ├─ 验证签名 Workflow、Signer SHA、Source SHA/ref 与托管 Runner
  ├─ 证明被修改的 Bundle 与错误 Source Digest 无法通过
  ├─ 拒绝不安全的 Archive 路径
  ├─ 验证所有文件 Hash 与 5 个可信输入 Hash
  ├─ 校验全部 Eval Result，并拒绝任何 fail 结果
  └─ 上传独立的 Verification Evidence
```
...
公开 Manifest 记录了 34 个证据文件与 8/8 通过结果。它不会删除或改写失败：
只要存在任意失败的 Eval Result，Verifier 就会拒绝该 Manifest。
...
Source Identity、可信输入、证据 Hash 与 Eval Result。
