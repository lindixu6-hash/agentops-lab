...
目前已完成 `[真实数据]` 位首测，任务完成率 `[真实数据]%`，中位耗时
`[真实数据]` 分钟。最关键的一条反馈是：`[经用户确认的匿名引语]`。

项目已开源，评论“AgentOps”获取 Demo 和测试数据。
...
AI Agent 上线后的运营，不能停留在“挑几条对话看起来不错”。本文分享一个本地优先的 AgentOps 实践：如何把 CSV / JSON 运行日志转成可复现指标，如何区分确定性计算与本地模型归纳，以及如何用 5 位目标用户验证这份报告是否真的支持版本放量决策。

完整文章：

- `docs/production-ai-skills-article.zh-CN.md`
...
基础指标用确定性代码计算，本地 Qwen + OpenVINO 只负责归纳聚合结果；原始日志不上传，也不回退云模型。
